...
- **阶段**:`开发完成` — 对应六步流程**第⑥步完成(CD 待服务器)**
- **上一步完成**:PR #1 已合并 main → CD 自动触发 → 预期失败(无服务器 Secrets)
- **下一步 (TODO 第一条)**:有服务器后配置 SSH Secrets,重新触发 CD
- **阻塞项**:暂无服务器;CD 需配 `SSH_PRIVATE_KEY`/`SSH_HOST`/`SSH_USER` 三个 Secrets
